...
public class ParticleEmitter {
	...
	public Particle[] particleTypesThatNeedData = new Particle[] { Particle.REDSTONE, Particle.ITEM_CRACK,
	 ...
	 *            (20 ticks = 1 second)
	 */
	public void emitParticlesContinuously(Entity entity, Particle particle, int amount, double speed, Vector spread,
	 ...
	 *            The entity at which the particles will be emitted
	 * @param offset
	 *            The offset from the entity where the particles will be emitted
	 * @param particle
	 *            The particle type to be emitted
	 * @param amount
	 *            How many particles should be emitted
	 ...
	 *            (20 ticks = 1 second)
	 */
	public void emitParticlesContinuously(Entity entity, Vector offset, Particle particle, int amount, double speed,
			Vector spread, Plugin plugin, int delay, int period, int duration) {
		...
		BukkitRunnable runnable = new BukkitRunnable() {
			...
			public void run() {
				...
				// Emit particles
				emitParticles(entity.getWorld(),
						LocationHelper.getInstance().offsetLocation(entity.getLocation(), offset), particle, amount,
						speed, spread);
	 ...
	 *            (20 ticks = 1 second)
	 */
	public void emitParticlesContinuously(Location location, Particle particle, int amount, double speed, Vector spread,
			Plugin plugin, int delay, int period, int duration) {
	 ...
	 *            The spread of the particles (using the entity location as a base)
	 */
	public void emitParticles(World world, Location location, Particle particle, int amount, double speed,
			Vector spread) {
		...
		for (Particle particleThatNeedsData : this.particleTypesThatNeedData) {
			...
			if (particle == particleThatNeedsData) {
				...
				Chat.getInstance().sendErrorToConsole("PluginBase", "Tried to emit particle without data",
	 ...
	 *            The spread of the particles (using the entity location as a base)
	 * @param dustOptions
	 *            The type of the particle dust
	 */
	public void emitParticles(World world, Location location, Particle particle, DustOptions dustOptions, int amount,
			double speed, Vector spread) {
		world.spawnParticle(particle, // Particle type
				location.getX(), // Location X
				location.getY(), // Location Y
				location.getZ(), // Location Z
	 ...
	 *            The spread of the particles (using the entity location as a base)
	 * @param blockData
	 *            The type of block the particles should belong to
	 */
	public void emitParticles(World world, Location location, Particle particle, BlockData blockData, int amount,
			double speed, Vector spread) {
		world.spawnParticle(particle, // Particle type
				location.getX(), // Location X
				location.getY(), // Location Y
