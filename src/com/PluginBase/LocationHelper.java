...
/**
 * This class helps with mixed things that have to do with location manipulation
 * and finding
 * 
...
public class LocationHelper {
	 ...
	 * 
	 * @param startLocation
	 *            The location that will be used as the center for the new offseted
	 *            location
	 * @param offset
	 *            The offset that should be added to the start location
	 * @return Offset location
	 */
	public Location offsetLocation(Location startLocation, Vector offset) {
		return new Location(startLocation.getWorld(), startLocation.getX() + offset.getX(),
				startLocation.getY() + offset.getY(), startLocation.getZ() + offset.getZ());
	}
	 ...
	 *            blocks
	 * @param range
	 *            The range in which blocks will be searched
	 * @param blockType
	 *            The type of block that is being searched for
	...
	public List<Block> findNearbyBlocks(Location startLocation, int range, Material blockType) {
		...
		// Prepare a list in which all the nearby blocks of the type blockType will be
		// saved
	 ...
	 * 
	 * @param location
	 *            The location that should be validated
	...
	/**
	 * Checks if a location is in the air
	 * 
	 * @param location
	 *            The location that should be checked
	 ...
	 *            positions
	 * @param minimumRange
	 *            The minimum range the new location has to be from the start
	 *            location
	 * @param maximumRange
	 *            The maximum range the new location may be from the start location
	...
	public Location getRandomNearbyPosition(Location startLocation, int range) {
		...
		if (tries >= maximumTries - 1) {
			Chat.getInstance().sendWarningMessageToConsole("PluginBase",
	...
	public List<Block> getBlocksBetweenLocations(Location location1, Location location2) {
		...
		/**
		 * Here we take the smallest location (put all of the smallest values from
		 * location1 and location2 into a new location) Then we get the biggest
		 * location. After this we can use a simple for loop to iterate from one to the
		 * other without worrying about direction
		 */
		// Get the smallest possible location
		Location startLocation = new Location(location1.getWorld(),
		...
		Location goalLocation = new Location(location1.getWorld(),
				(location1.getX() < location2.getX()) ? location2.getX() : location1.getX(),
				(location1.getY() < location2.getY()) ? location2.getY() : location1.getY(),
				(location1.getZ() < location2.getZ()) ? location2.getZ() : location1.getZ());
